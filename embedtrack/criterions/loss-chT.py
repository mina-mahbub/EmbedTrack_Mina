...
class EmbedTrackLoss(nn.Module):
    ...
    def forward(
        ...
        # Ensure that loss is initialized as a tensor on the correct device
        loss = torch.tensor(0.0, device=device)
        track_loss = torch.tensor(0.0, device=device)
        track_count = 0

        # Initialize loss values as tensors to avoid scalar issues
        loss_values = {
        ...
        for b in range(0, min(batch_size, offset_predictions.size(0))):
            ...
            seed_loss = torch.tensor(0.0, device=device)
            ...
            for i, inst_id in enumerate(instance_ids):
                ...
                var_loss += torch.mean(torch.pow(sigma_in - s.detach(), 2))
                ...
                # apply lovasz-hinge loss
                instance_loss += lovasz_hinge(dist * 2 - 1, in_mask.to(device))
                ...
                if b < (batch_size // 2):
                    ...
                    track_loss += lovasz_hinge(dist_tracking * 2 - 1, in_mask.to(device))
                    track_count += 1
            ...
            # accumulate losses and ensure they are tensors
            loss += w_inst * instance_loss + w_var * var_loss + w_seed * seed_loss
            loss_values["instance"] += w_inst * instance_loss.detach()
            loss_values["variance"] += w_var * var_loss.detach()
            loss_values["seed"] += w_seed * seed_loss.detach()

        # Normalize track_loss safely
        if track_count > 0:
            track_loss /= track_count
        loss += track_loss
        loss_values["track"] += track_loss.detach()
