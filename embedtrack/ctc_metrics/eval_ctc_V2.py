...
# result_dir = Path("/home/MinaHossain/EmbedTrack/ctc_raw_data/challenge/Cell-Data-P2/02_RES_FW")
...
def select_platform():
    """Selects the correct executables for the cell tracking challenge measure."""
    sys_type = platform.system()
    if sys_type == "Linux":
        return "Linux"
    if sys_type == "Windows":
...
def validate_data_file(file_path):
    """Validation to check for missing data or inconsistencies in a tracking result file."""
    with open(file_path, 'r') as file:
        ...
        for idx, line in enumerate(lines):
            ...
            if len(values) < 4:
                raise ValueError(f"Data inconsistency detected on line {idx+1} of {file_path}")

            # Validation for parent-child relationship
            label, parent = values[0], values[1]
            if parent != 0:  # Check for non-root elements
                if parent not in parent_child_map:
                    raise ValueError(
                        f"Invalid parent connection at line {idx+1}: Parent {parent} does not exist for label {label}"
                    )
                parent_child_map[label] = parent
            else:
                parent_child_map[label] = None  # Root element
    return True
...
def calc_tra_score(result_dir, gt_dir, get_aogm_results=True):
    ...
    platform_name = select_platform()
    tra_measure_file = [
        file for file in os.listdir(Path(tra_measure_dir, platform_name)) if file.startswith("TRAMeasure")
    ]
    assert len(tra_measure_file) == 1, "Only one tracking measure executable per subdirectory allowed"
    tra_measure_path = Path(tra_measure_dir, platform_name, tra_measure_file[0])
    data_dir = gt_dir.parent
    data_set_id = gt_dir.name.split("_")[0]

    # Validate input data files
    validate_data_file(result_dir / "res_track.txt")
    validate_data_file(gt_dir / "TRA" / "man_track.txt")
...
def calc_ctc_scores(result_dir, gt_dir):
    ...
    Returns: dictionary containing the metric names and their scores
    """
    ctc_measure_dir = Path(__file__).parent / "CTC_eval"
    assert Path.exists(ctc_measure_dir), "Missing folder with CTC measures"
    platform_name = select_platform()
    measure_files = [
        file for file in os.listdir(Path(ctc_measure_dir, platform_name)) if "Measure" in file
    ]
    ...
    n_files = max(
        [int(re.findall(r"\d+", f)[0]) for f in os.listdir(gt_dir / "TRA") if f.endswith("tif")]
    )
    n_digits = 4
    ...
    for measure_f in measure_files:
        ...
        ctc_metric_path = Path(ctc_measure_dir, platform_name, measure_f)
        output = subprocess.Popen(
            [ctc_metric_path.as_posix(), data_dir.as_posix(), data_set_id, str(n_digits)],
            stdout=subprocess.PIPE,
        )
        result, _ = output.communicate()
        metric_score = re.findall(r"\d\.\d*", result.decode("utf-8"))
...
def extract_aogm_info(tra_log_file):
    """Extracts the number of different tracking errors from the TRA log file."""
    pattern = re.compile(r"\[*T=\d+")
    with open(tra_log_file, "r") as f:
        lines = f.readlines()
        error_counts = {"EdgeSplit": 0, "EdgeAdd": 0, "EdgeDelete": 0, "EdgeSemantic": 0, "FP": 0, "FN": 0}
        errors_over_t = {key: {} for key in error_counts}
        counter_key = None
        for line in lines:
            match = re.match(pattern, line)
            if match and counter_key:
                time_id = int(match.group().split("=")[-1])
                errors_over_t[counter_key].setdefault(time_id, 0)
                errors_over_t[counter_key][time_id] += 1
            if "Splitting" in line:
                counter_key = "EdgeSplit"
            elif "False Negative Vertices" in line:
                ...
                if counter_key:
                    error_counts[counter_key] += 1
        return {"counts": error_counts, "t": errors_over_t}
